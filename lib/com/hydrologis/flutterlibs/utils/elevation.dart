// This comes from the now unmaintained map_elevation project
part of smashlibs;
          ...
          onHorizontalDragUpdate: (DragUpdateDetails details) {
            ...
            if (pointFromPosition != null) {
              ...
              setState(() {
                ...
                _hoveredAltitude = pointFromPosition.altim;
              });
            }
          },
...
class _ElevationPainter extends CustomPainter {
  ...
  _ElevationPainter(this.points,
    ...
    _min = (points.map((point) => point.altim).toList().reduce(min) / 100)
            .floor() *
        100;
    _max =
        (points.map((point) => point.altim).toList().reduce(max) / 100).ceil() *
            100;

    _relativeAltitudes =
        points.map((point) => (point.altim - _min) / (_max - _min)).toList();
  }
  ...
  void paint(Canvas canvas, Size size) {
    ...
    if (elevationGradientColors != null) {
      ...
      for (int i = 1; i < points.length; i++) {
        double dX = Distance().distance(points[i], points[i - 1]);
        double dZ = (points[i].altim - points[i - 1].altim);
  ...
  LatLngExt? getPointFromPosition(double position) {
