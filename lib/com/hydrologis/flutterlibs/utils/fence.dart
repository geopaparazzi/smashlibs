  ...
  Future<Fence?> showFencePropertiesDialog(
    ...
    return await showDialog<Fence>(
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          title:
              Text(SLL.of(context).fence_fenceProperties), //"Fence Properties"
          ...
          actions: <Widget>[
            if (allowDelete)
              TextButton(
                child: Text(
                  SLL.of(context).fence_delete, //"DELETE"
                  style: TextStyle(color: SmashColors.mainDanger),
                ),
                onPressed: () async {
                  var res = await SmashDialogs.showConfirmDialog(
                          context,
                          SLL.of(context).fence_removeFence, //"Remove fence"
                          "${SLL.of(context).fence_areYouSureRemoveFence} ${fence.name}") //"Are you sure you want to remove fence:"
                      ??
                      false;
                  if (res) {
            ...
            TextButton(
              child: Text(SLL.of(context).fence_cancel), //"CANCEL"
              onPressed: () {
                Navigator.of(context).pop(null);
              },
            ),
            TextButton(
              child: Text(SLL.of(context).fence_ok), //"OK"
              onPressed: () {
...
class Fence {
  ...
  Fence(BuildContext context)
      : creationTs = DateTime.now().millisecondsSinceEpoch,
        name = SLL.of(context).fence_aNewFence; //"a new fence"
...
class _FencePropertiesContainerState extends State<FencePropertiesContainer> {
  ...
  Widget build(BuildContext context) {
    ...
    var nameID = new InputDecoration(
        labelText: SLL.of(context).fence_label, //"Label"
        hintText:
            SLL.of(context).fence_aNameForFence); //"A name for the fence."
    ...
    var radiusID = new InputDecoration(
        labelText: SLL.of(context).fence_radius, //"Radius"
        hintText: SLL
            .of(context)
              ...
              children: [
                Padding(
                  ...
                  child: SmashUI.smallText(
                      SLL.of(context).fence_onEnter), //"On enter"
                ),
            ...
            child: Row(
              children: [
                Padding(
                  padding: SmashUI.defaultRigthPadding() * 2,
                  child: SmashUI.smallText(
                      SLL.of(context).fence_onExit), //"On exit"
                ),
                DropdownButton<ENotificationSounds>(
                  items: ENotificationSounds.values
                      .map((ns) => DropdownMenuItem<ENotificationSounds>(
